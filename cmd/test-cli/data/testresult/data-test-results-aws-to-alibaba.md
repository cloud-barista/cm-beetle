# CM-Beetle test results for ALIBABA ap-northeast-2 (data migration)

> [!NOTE]
> This document presents test results for CM-Beetle data migration to ALIBABA (ap-northeast-2).
...
- Test Time: 19:58:49 KST
- Test Execution: 2026-06-24 19:58:49 KST
...
## Test result for ALIBABA ap-northeast-2

### Test Results Summary

| Step | Endpoint / Description | Status | Duration | Details |
|------|------------------------|--------|----------|---------|
| 1 | `POST /beetle/migration/middleware/ns/mig01/objectStorage` (target) | ✅ **PASS** | 2.532s | Pass |
| 2 | `POST /beetle/migration/data` (migrate: source OS → target OS, encrypted, async) | ✅ **PASS** | 10.15s | Pass |
| 3 | Verify migrated data (compare source and target object lists) | ✅ **PASS** | 1.047s | Pass |
| 4 | `DELETE /beetle/migration/middleware/ns/mig01/objectStorage/{targetOsId}` (cleanup) | ✅ **PASS** | 5.417s | Pass |

**Overall Result**: 4/4 steps passed ✅

**Total Duration**: 25.1492487s

*Test executed on June 24, 2026 at 19:58:49 KST (2026-06-24 19:58:49 KST) using CM-Beetle automated test CLI*
...
- **Status**: ✅ **SUCCESS**
- **Duration**: 10.15s
- **Note**: Initial response was `202 Accepted` (status: `Handling`). The test CLI polled `GET /beetle/request/1782298737848072099` every 10s (timeout: 600s) until migration completed with status `Success`. Step 3 was then executed.
<details>
...
{
  "data": {
    "reqId": "1782298737848072099",
    "status": "Handling",
    "statusUrl": "/beetle/request/1782298737848072099"
  },
