...
func GenerateMigrationReportMarkdown(report *MigrationReport) string {
	...
	// Header
	md.WriteString("# 🚀 Infrastructure Migration Report\n\n")
...
func writeExecutiveSummary(md *strings.Builder, summary *ExecutiveSummary, metadata *ReportMetadata) {
	md.WriteString("## 📊 Migration Summary\n\n")

	// Target Cloud Information (최우선)
	...
	if summary.FailedServers > 0 {
		md.WriteString(fmt.Sprintf("⚠️ **Failed Servers:** %d\n\n", summary.FailedServers))
	}

	// Cost Estimate
	if summary.MonthlyCostUSD > 0 {
		md.WriteString(fmt.Sprintf("**💰 Estimated Monthly Cost:** $%.2f USD\n\n", summary.MonthlyCostUSD))
	}
...
func writeMigratedResourcesOverview(md *strings.Builder, report *MigrationReport) {
	md.WriteString("## 📦 Migrated Resources Overview\n\n")
	...
	vmSpecStatus := "✅ Selected"
	vmSpecCountStr := fmt.Sprintf("%d", vmSpecCount)
	if vmSpecCount == 0 {
		vmSpecStatus = "⚠️ Not selected"
		vmSpecCountStr = "0"
	}
	...
	vmImageStatus := "✅ Selected"
	vmImageCountStr := fmt.Sprintf("%d", vmImageCount)
	if vmImageCount == 0 {
		vmImageStatus = "⚠️ Not selected"
		vmImageCountStr = "0"
	}
...
func writeVmMigrationStatus(md *strings.Builder, report *MigrationReport) {
	md.WriteString("## 💻 Virtual Machines (VMs)\n\n")
...
func writeMigrationMappings(md *strings.Builder, mappings []SourceTargetMapping) {
	md.WriteString("## 🔄 Migration Mappings\n\n")
	...
	for i, mapping := range mappings {
		...
		// Machine/Instance ID
		md.WriteString(fmt.Sprintf("| **ID** | %s | %s | ✅ Mapped |\n",
		...
		sourceCPUInfo := fmt.Sprintf("%d CPUs × %d threads", mapping.SourceServer.CPUs, mapping.SourceServer.CPUThreads)
		if mapping.SourceServer.CPUModel != "" {
			sourceCPUInfo = fmt.Sprintf("%d CPUs × %d threads (%s)", mapping.SourceServer.CPUs, mapping.SourceServer.CPUThreads, mapping.SourceServer.CPUModel)
		}
		...
		if mapping.CostPerMonth > 0 {
			md.WriteString(fmt.Sprintf("**💰 Monthly Cost:** $%.2f USD\n\n", mapping.CostPerMonth))
		}
...
func writeResourceChanges(md *strings.Builder, changes *ResourceChangeAnalysis) {
	md.WriteString("#### 📈 Resource Changes\n\n")
...
func writeNetworkAnalysis(md *strings.Builder, analysis *NetworkMigrationAnalysis) {
	md.WriteString("## 🌐 Network Migration Analysis\n\n")
	...
	if analysis.CIDRPreserved {
		md.WriteString("✅ CIDR preserved in target VNet\n\n")
	} else {
		md.WriteString("⚠️ CIDR changed in target VNet\n\n")
	}
...
func writeSecurityAnalysis(md *strings.Builder, analysis *SecurityMigrationAnalysis) {
	md.WriteString("## 🔒 Security Migration Analysis\n\n")
...
func formatResourceChange(source, target int) string {
	if target == source {
		...
		return fmt.Sprintf("↗️ +%d (+%.1f%%)", target-source, pct)
	} else {
		pct := calculatePercentageChange(source, target)
		return fmt.Sprintf("↘️ %d (%.1f%%)", target-source, pct)
	}
...
// formatCPUMatching analyzes CPUs × threads to vCPU matching and returns status string
func formatCPUMatching(sourceCPUs int, sourceThreads int, targetVCPUs int) string {
	// Actual mapping: CPUs (sockets) × CPU Threads = Expected vCPUs
	// Example: 1 CPU × 16 threads = 16 vCPUs
	// Scenarios:
	// - Perfect match: source CPUs × threads = target vCPUs
	// - Overprovisioned: target vCPUs > source CPUs × threads
	// - Underprovisioned: target vCPUs < source CPUs × threads

	expectedVCPUs := sourceCPUs * sourceThreads

	if targetVCPUs == expectedVCPUs {
		return fmt.Sprintf("✅ Matched (%d×%d=%d)", sourceCPUs, sourceThreads, expectedVCPUs)
	} else if targetVCPUs > expectedVCPUs {
		extra := targetVCPUs - expectedVCPUs
		pct := float64(extra) / float64(expectedVCPUs) * 100
		return fmt.Sprintf("↗️ +%d vCPUs (+%.0f%%) [expected: %d×%d=%d]", extra, pct, sourceCPUs, sourceThreads, expectedVCPUs)
	} else {
		deficit := expectedVCPUs - targetVCPUs
		pct := float64(deficit) / float64(expectedVCPUs) * 100
		return fmt.Sprintf("↘️ -%d vCPUs (-%.0f%%) [expected: %d×%d=%d]", deficit, pct, sourceCPUs, sourceThreads, expectedVCPUs)
	}
}
